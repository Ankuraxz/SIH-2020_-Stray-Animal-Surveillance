...
loc ="x" # predefined location here
...
def sms(tag,prob,loc,date,time,cam,to_tx):
    ...
    for _ in len(tag):
        tag1 = str(tag[_])
        prob1 = str(prob[_])
        msg = "Animal"+tag1+"is detected "+ cam +" with a confidence of"+prob1 +"at location"+loc+"on"+ date +"at"+ time
        message = client.messages.create(
            body = msg,
            to = to_tx,
            from_ ='+19892828077') # from is predefined


def email(tag,prob,loc,date,time,cam,email_rx):
    ...
    for _ in len(tag):
        tag1 = str(tag[_])
        prob1 = str(prob[_])
        msg_ = "Animal"+tag1+"is detected in"+ cam +"with a confidence of"+prob1 +"at location"+loc+"on"+ date +"at"+ time+" Image attached"
...
if (tags !=[] and prob !=[]): # if some animal detected
    ...
    sms(tags,prob,loc,date,time,cam_name,to)

    # sending email alert with pic
    to_ =["ankurvermaaxz@gmail.com","ruchikamodgil@gmail.com"] # mail to send alert
    email(tags,prob,loc,date,time,cam_name,to_)
